...
const selectPair = (e) => {
  // หาเป้าหมายที่ใกล้ที่สุดที่มี data-code
  ...
  if (target) {
    ...
    state[openedDrawer] = target.dataset.code;
...
// อัพเดท HTML template ในฟังก์ชัน displayCurrencies
const displayCurrencies = () => {
  ui.currencyList.innerHTML = state.filteredCurrencies
    .map(({ code, name }) => {
      return `
      <li data-code="${code}" role="button" tabindex="0">
